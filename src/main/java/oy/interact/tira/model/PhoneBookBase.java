...
public abstract class PhoneBookBase {
	...
	protected Coder coderFromSearchString(final String toSearch, final CoderSortOrder sortOrder) {
		Coder coder = null;
		if (sortOrder == null ||
				sortOrder == CoderSortOrder.FULLNAME_ASCENDING ||
				sortOrder == CoderSortOrder.FULLNAME_DESCENDING) {
			...
			coder = new Coder(UUID.randomUUID().toString());
			coder.setCoderName(toSearch);
		}
		return coder;
	}
	...
	public void checkJSONFile(File selectedFile) throws IOException, ParenthesesException {
		...
		if (null != charStack) {
			...
			addMeasurement(String.format("Checking parenthesis from %s ", selectedFile.getName()), duration);
		}
	}
